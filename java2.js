...
let playerReady = false; // YouTube player ready state
let currentSlide = 0;
const totalSlides = 8;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeModal();
    initializeParallax();
    // Pre-cargar la API de YouTube para poder reproducir al hacer clic
    try { loadYouTubeAPI(); } catch (_) {}
...
function initializeModal() {
    ...
    enterInvitation.addEventListener('click', function() {
        ...
        // Activar música y reproducir desde YouTube en el clic (gesto del usuario)
        enableMusic = true;
        // Mostrar el control de música inmediatamente para dar feedback al usuario
        try {
        ...
        try {
            if (player && typeof player.playVideo === 'function') {
                try { if (typeof player.unMute === 'function') player.unMute(); } catch (_) {}
                try { if (typeof player.setVolume === 'function') player.setVolume(100); } catch (_) {}
                player.playVideo();
                isPlaying = true;
                updateMusicIcon();
            } else if (window.YT && window.YT.Player) {
                // Crear el player ahora y se reproducirá en onPlayerReady
                initializeYouTubePlayer();
            } else {
                // Cargar la API si aún no está disponible
                loadYouTubeAPI();
            }
        } catch (_) {}
    });
...
function loadYouTubeAPI() {
    ...
    window.onYouTubeIframeAPIReady = function() { initializeYouTubePlayer(enableMusic); };
}

// Función llamada por la API de YouTube
function initializeYouTubePlayer(autoplayOnCreate = false) {
    if (player) return; // evitar crear múltiples instancias
...
function onPlayerReady(event) {
    ...
    // Asegurar permisos de autoplay en el iframe
    try {
    ...
    // Reproducir solo si ya se eligió música (click del usuario)
    if (enableMusic) {
...
function onPlayerStateChange(event) {
    if (event.data === YT.PlayerState.PLAYING) {
        try {
            isPlaying = player && typeof player.isMuted === 'function' ? !player.isMuted() : true;
        } catch (_) {
            isPlaying = true;
        }
    } else if (
